                   ...
                   const size_t dev_mem_base_addr_align)
{
  ...
  cl_mem out_sub_buf_ = clCreateSubBuffer(out_buf,
                                          CL_MEM_WRITE_ONLY,
...
cl_int
test_apply_rescue_permutation(cl_context ctx,
                              cl_command_queue cq,
                              cl_kernel krnl)
{
  cl_int status;
  ...
  cl_event evt_4;
  status = clEnqueueNDRangeKernel(
    cq, krnl, 1, NULL, global_size, local_size, 4, evts, &evt_4);

  cl_event evt_5;
  status = clEnqueueReadBuffer(
    cq, out_buf, CL_FALSE, 0, sizeof(out_arr), out_arr, 1, &evt_4, &evt_5);
...
cl_int
test_apply_mds(cl_context ctx, cl_command_queue cq, cl_kernel krnl)
{
  cl_int status;
  ...
  cl_event evt_2;
  status = clEnqueueNDRangeKernel(
    cq, krnl, 1, NULL, global_size, local_size, 2, evts, &evt_2);

  cl_event evt_3;
  status = clEnqueueReadBuffer(
    cq, out_buf, CL_FALSE, 0, sizeof(out_arr), out_arr, 1, &evt_2, &evt_3);
...
cl_int
test_reduce_sum_vec2(cl_context ctx, cl_command_queue cq, cl_kernel krnl)
{
  cl_int status;
  ...
  cl_mem in_buf =
    clCreateBuffer(ctx, CL_MEM_READ_ONLY, sizeof(cl_ulong) * 16, NULL, &status);
  cl_mem out_buf =
    clCreateBuffer(ctx, CL_MEM_WRITE_ONLY, sizeof(cl_ulong) * 8, NULL, &status);
              ...
              cl_ulong* ts)
{
  ...
  cl_event evt_0;
  status = clEnqueueWriteBuffer(
    cq, in_buf, CL_FALSE, 0, in_size, input, 0, NULL, &evt_0);
  ...
  cl_event evt_2;
  status = clEnqueueWriteBuffer(
    cq, mds_buf, CL_FALSE, 0, sizeof(MDS), MDS, 0, NULL, &evt_2);
  cl_event evt_3;
  status = clEnqueueWriteBuffer(
    cq, ark1_buf, CL_FALSE, 0, sizeof(ARK1), ARK1, 0, NULL, &evt_3);
  cl_event evt_4;
  status = clEnqueueWriteBuffer(
    cq, ark2_buf, CL_FALSE, 0, sizeof(ARK2), ARK2, 0, NULL, &evt_4);
  ...
  cl_event evt_5;
  status = clEnqueueNDRangeKernel(
    cq, krnl, 2, NULL, global_size, local_size, 5, evts, &evt_5);

  // hash output being copied back to host
  cl_event evt_6;
  status = clEnqueueReadBuffer(
    cq, out_buf, CL_FALSE, 0, out_size, output, 1, &evt_5, &evt_6);
  ...
  if (ts != NULL) {
    cl_ulong start, end;
    status = clGetEventProfilingInfo(
      evt_5, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &start, NULL);
    check(status);
    status = clGetEventProfilingInfo(
      evt_5, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &end, NULL);
    check(status);
      ...
      cl_ulong* ts)
{
  ...
  cl_event evt_4;
  status = clEnqueueNDRangeKernel(
    cq, krnl, 2, NULL, global_size, local_size, 4, evts, &evt_4);

  // hash output being copied back to host
  cl_event evt_5;
  status = clEnqueueReadBuffer(
    cq, out_buf, CL_FALSE, 0, out_size, output, 1, &evt_4, &evt_5);
